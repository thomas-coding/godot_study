...
- `可验证性`：有明确通过/失败观测点。
- `可恢复性`：失败时可最小代价回退。
- `代码可复制性`：GDScript 代码块必须按 Godot 可直接粘贴格式给出；顶级代码不得带多余前导空格，函数体缩进优先使用 Tab，并明确说明“替换整个函数”或“插入到哪一行同级位置”。代码块必须独立成段，不能嵌在编号列表或项目符号缩进内；禁止给会被学员误复制进项目的演示函数。若只是说明格式，用纯文字说明，不给可运行形态的示例函数。
