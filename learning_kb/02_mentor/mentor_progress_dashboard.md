...
| --- | --- | --- | --- | --- | --- |
| D1 Runtime & Core API Mastery | 84 | 20% | 16.8 | M01-M10、K001-K041、QA001-QA040 | 需扩展到 UI/Signals/渲染深水区 |
| D2 Feature Solution Design | 70 | 20% | 14.0 | playbook 扩展到 F001~F035 | 需补跨平台/发布链路决策树 |
| D3 Source-level Q&A Speed | 78 | 20% | 15.6 | quick-answer map v1 已扩展到 70 问题 | 需继续压缩答复路径并补渲染源码链路 |
| D4 Engineering Reliability | 38 | 20% | 7.6 | 排错清单 + 性能案例库 + 自动回归规范 + 真实构建回归样本 | 缺自动化执行样本与阈值调优 |
| D5 Shipping & Version Governance | 32 | 20% | 6.4 | 版本治理 + preflight + 风险矩阵 + 构建证据链已落地 | 缺 Steam 上线前端到端演练与指标闭环 |
...
- Sprint Goal: `60% -> 64%`
- Must Deliver:
  1. 为渲染/资源管线再补 10 个 quick-answer 条目（70 -> 80）
  2. playbook 从 F035 扩展到 F040（补 Steam/导出决策）
  3. 真实构建样本累计到 5 次并建立阈值带（均值与波动区间）
